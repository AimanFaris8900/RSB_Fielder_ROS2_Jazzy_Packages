...
1) Launch Fielder SLAM mapping
```bash
ros2 launch fielder_navigation mapping.launch.py 
```
2) **(Optional)** Launch Fielder SLAM mapping with the origin relative to robot base
...
2) Keep the rviz2 UI open, run nav2 bringup
```bash
ros2 launch nav2_bringup navigation_launch.py 
```

3) On rviz2, go to Panels (top left) and click Add New Panel. Then select Navigation2 under nav2_rviz_plugins then click OK

4) Make sure navigation is active. Click plus (+) top right, then select GoalTool under nav2_rviz_plugins

5) To make Fielder navigate to selected points, select Nav2 Goal (top) then select points on the map with its orientation/heading
